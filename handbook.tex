\documentclass[twoside,makeidx]{book}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{xspace}
\usepackage[clean]{svg}
\usepackage{combelow}
\usepackage{newunicodechar}
\newunicodechar{ș}{\cb{s}}
\newunicodechar{ț}{\cb{t}}

%% \usepackage{fontspec,xunicode,xltxtra}
%% \setromanfont[Mapping=tex-text]{Times}
%% \setsansfont[Mapping=tex-text]{Lucida Grande}
%% \setmonofont{Courier New}

\usepackage{pdfpages}
\usepackage[backend=biber]{biblatex}
% set paper size and margins; needs to be adapted for A5 ideally, we
% should have a document class for conference handbooks where A5
% vs. letter-halved is a class option
\usepackage[
  paperheight=8.5in,
  paperwidth=5.5in,
  inner=.75in,
  outer=.5in,
  bottom=.75in,
  top=.75in,
  twoside]{geometry}

% Lots of macros
\input{content/special/preamble}
\input{content/special/macros}

\input{content/setup/venues}    % macros for event locations
\input{content/setup/sessions}  % session titles and venues

\renewcommand{\normalsize}{\fontsize{8}{9}\selectfont}
\renewcommand{\small}{\fontsize{7}{8}\selectfont}
\renewcommand{\footnotesize}{\fontsize{6}{6}\selectfont}
\renewcommand{\large}{\fontsize{10}{11}\selectfont}
\renewcommand{\Large}{\fontsize{12}{14}\selectfont}
\renewcommand{\huge}{\fontsize{14}{17}\selectfont}

% for each part of the conference, add the .bib file produced with
% meta2bibtex.py here:

%\bibliography{auto/tacl/papers.bib}

%\bibliography{auto/argmining/papers.bib}
%\bibliography{auto/bea/papers.bib}
%\bibliography{auto/clfl/papers.bib}
%\bibliography{auto/clpsych/papers.bib}
%\bibliography{auto/cmcl/papers.bib}
%\bibliography{auto/demos/papers.bib}
%\bibliography{auto/event/papers.bib}
%\bibliography{auto/exprom/papers.bib}
%\bibliography{auto/law/papers.bib}
%\bibliography{auto/metaphor/papers.bib}
%\bibliography{auto/mwe/papers.bib}
%\bibliography{auto/papers/papers.bib}
%\bibliography{auto/semeval/papers.bib}
%\bibliography{auto/socialnlp/papers.bib}
%\bibliography{auto/srw/papers.bib}
%\bibliography{auto/ssst-9/papers.bib}
%\bibliography{auto/starsem/papers.bib}
%\bibliography{auto/tutorial-final/papers.bib}
%\bibliography{auto/vsm-nlp/papers.bib}
\bibliography{auto/argmining/papers.bib}
\bibliography{auto/blackbox/papers.bib}
\bibliography{auto/ci+nlp/papers.bib}
\bibliography{auto/codi/papers.bib}
\bibliography{auto/conll/papers.bib}
\bibliography{auto/crac/papers.bib}
\bibliography{auto/eancs/papers.bib}
\bibliography{auto/econlp/papers.bib}
\bibliography{auto/eval4nlp/papers.bib}
\bibliography{auto/fever/papers.bib}
\bibliography{auto/insights/papers.bib}
\bibliography{auto/latech-clfl/papers.bib}
\bibliography{auto/lawdmr/papers.bib}
\bibliography{auto/MRL/papers.bib}
\bibliography{auto/mrqa/papers.bib}
\bibliography{auto/newsum/papers.bib}
\bibliography{auto/nllp/papers.bib}
\bibliography{auto/nlp4convai/papers.bib}
\bibliography{auto/papers/papers.bib}
\bibliography{auto/sustainlp/papers.bib}
\bibliography{auto/WiNLP/papers.bib}
\bibliography{auto/w-nut/papers.bib}
% Special bib for the workshops
\bibliography{content/workshops/papers.bib}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}

\setdaydateyear{Sunday}{November 7}{2021}

%% COVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\fancyfoot[C]{}
\includepdf[pages={1}]{content/fmatter/cover.pdf}

%% INSIDE FRONT COVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{content/maps}

\cleardoublepage
\thispagestyle{empty}
%\vspace*{6in}
\emph{Cover by Nathan Cornille}\index{Cornille, Nathan}\\
%\emph{Deep gratitude to Joachim Bingel for invaluable advice with creating this handbook}\index{Bingel, Joachim}
    \emph{Handbook assembled by Els Lefever, Pranaydeep Singh and Loic De Langhe}\index{Lefever, Els}\index{Singh, Pranaydeep}\index{De Langhe, Loic}\\
\emph{Printing by Omnipress of Madison, Wisconsin}\\

\newpage
\cleardoublepage
\fancyfoot[C]{\thepage}
\frontmatter

%% SPONSORS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Putting on back cover

%% \addcontentsline{toc}{chapter}{Sponsorship}
%% \thispagestyle{empty}
%% \setheaders{}{}
%% \input{content/sponsors/sponsors}

%% TOC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\addcontentsline{toc}{chapter}{Table of Contents}
\setcounter{tocdepth}{2}
\tableofcontents
\mainmatter
\pagestyle{fancy}


%% PREFACE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\chapter{Conference Information}

\input{content/fmatter/preface-chair}
\clearpage
\input{content/fmatter/preface-pc}

\clearpage%{\thispagestyle{emptyheader}\cleardoublepage}
\setheaders{}{}
\input{content/fmatter/committee}

%% \clearpage
%\input{content/fmatter/sponsors}
\clearpage%{\thispagestyle{emptyheader}\cleardoublepage}
\setheaders{}{}

%\input{content/meal-info.tex}
%\clearpage

%% Welcome reception %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% \addcontentsline{toc}{chapter}{Welcome Reception: \daydate, 6:00pm}
\setdaydateyear{Saturday}{November 6}{2021}
\input{content/tutorials/welcome_reception}

%% TUTORIALS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\setdaydateyear{Wednesday}{November 10}{2021}
\input{content/tutorials/tutorials-overview-1}

\setdaydateyear{Thursday}{November 11}{2021}
\input{content/tutorials/tutorials-overview-2}

%% MAIN CONFERENCE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\setdaydateyear{Sunday}{November 7}{2021}
\input{content/day1/day1}

\setdaydateyear{Monday}{November 8}{2021}
\input{content/day2/day2}

\setdaydateyear{Tuesday}{November 9}{2021}
\input{content/day3/day3}

%% Cultural event %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% \setdaydateyear{Thursday}{June 4}{2015}
%% \addcontentsline{toc}{chapter}{Special Cultural Event: Thursday, June 26, 8:00 PM}
%% \input{content/special-event}

%% *SEM% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%\setdaydateyear{Thursday--Friday}{June 4--5}{2015}
%\input{content/colocated/overview}

%% WORKSHOPS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\setdaydateyear{Thursday--Friday}{November 10--11}{2021}
\input{content/workshops/workshops}

%% MISC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Anti-harassment policy
\input{content/anti-harassment-policy}

\cleardoublepage
\addcontentsline{toc}{chapter}{Author Index}
\setheaders{Author Index}{Author Index}

\printindex

%% LOCAL GUIDE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\setheaders{Local Guide}{Local Guide}
\input{content/local-guide/local-guide.tex}

%% ADS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\clearpage
%\addcontentsline{toc}{chapter}{Sponsorship}
\setheaders{}{}
\input{content/ads/ads}


%% BACK COVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\thispagestyle{empty}

\noindent EMNLP 2021 gratefully acknowledges the following sponsors for
their support:\\

\input{content/sponsors/sponsors}

%% \input{content/restaurant-summary}
%% \includepdf[pages={1}]{content/backcover.pdf}

\end{document}

